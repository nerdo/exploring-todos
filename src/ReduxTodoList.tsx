...
import { addItems, editItems, deleteItems } from './redux/todoSlice'
import { useSelector, useDispatch } from './redux/reactHooks'
...
export const ReduxTodoList = () => {
  ...
  const actions = {
    addItems: useCallback((...items: IListItem[]) => dispatch(addItems(items)), [dispatch]),
    editItems: useCallback((...newItems: IListItem[]) => dispatch(editItems(newItems)), [dispatch]),
    deleteItems: useCallback((...itemIds) => dispatch(deleteItems(itemIds)), [dispatch])
  }

  return <PureTodoList listItems={listItems} {...actions} />
}
